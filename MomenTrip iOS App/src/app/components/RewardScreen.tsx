...
import { ChevronLeft, TrendingDown, TrendingUp, Store, Compass } from 'lucide-react';
import type { RewardHistoryItem, AppScreen } from '../App';

interface Props {
  onBack: () => void;
  onLocalCurrency: () => void; // 지역화폐 화면(15개 지역선택)으로 이동
  totalPoints: number;
  history: RewardHistoryItem[];
  onOpenMarketGuide: () => void; // 주변 전통시장 및 가맹점 안내(주요 관광 API 화면)로 이동
  onNavigate: (screen: AppScreen) => void; // 추가됨: 화면 이동 함수
}

export function RewardScreen({ onBack, onLocalCurrency, totalPoints, history, onOpenMarketGuide, onNavigate }: Props) {
    ...
    >
      ...
      <div className="px-5 pt-4 pb-3 flex-shrink-0 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p style={{ fontSize: 11, color: '#9E8B7E' }}>여행 미션 보상 및 가맹점 안내</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-4">
        ...
        {/* 1. 주변 전통시장 및 가맹점 안내 (주요 관광 API 화면으로 이동) */}
        <div 
          onClick={onOpenMarketGuide}
          className="rounded-2xl p-4 mb-3 flex items-center justify-between cursor-pointer active:scale-98 transition-all"
          style={{ background: '#F5EFE6', border: '1px solid #E3D5C6' }}
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: '#E3D5C6' }}>
              <Store size={20} color="#C97C56" />
            </div>
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#2A1F1A' }}>주변 전통시장 및 가맹점 안내</p>
        ...
        {/* 2. 새롭게 추가된 '지역별 관광 다양성' 블록 (관광객 다양성 및 소비 다양성 API 화면으로 이동) */}
        <div 
          onClick={() => onNavigate('localcurrency')}
          className="rounded-2xl p-4 mb-5 flex items-center justify-between cursor-pointer active:scale-98 transition-all"
          style={{ background: '#F5EFE6', border: '1px solid #E3D5C6' }}
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: '#E3D5C6' }}>
              <Compass size={20} color="#C97C56" />
            </div>
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#2A1F1A' }}>지역별 관광 다양성</p>
              <p style={{ fontSize: 11, color: '#9E8B7E', marginTop: 1 }}>관광객 다양성 및 소비 다양성 API 실시간 연동</p>
            </div>
          </div>
          <span style={{ fontSize: 12, fontWeight: 700, color: '#C97C56' }}>보기 &gt;</span>
        </div>
